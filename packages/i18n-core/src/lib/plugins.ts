 ...
 * YYC³ i18n Core - Extensibility Layer
 */
...
export class PluginManager {
  ...
  register(plugin: I18nPlugin): void {
    if (this.plugins.has(plugin.name)) {
      console.warn(`[i18n] Plugin "${plugin.name}" is already registered. Overwriting.`);
    }
    ...
    console.log(`[i18n] ✅ Plugin "${plugin.name}" registered${plugin.version ? ` v${plugin.version}` : ''}`);
  }
  ...
  unregister(name: string): boolean {
    ...
    console.log(`[i18n] 🗑️ Plugin "${name}" unregistered`);
    return true;
  }
